  ...
  return (
    <div className={`flex flex-col items-center ${isFirst ? 'order-2' : rank === 2 ? 'order-1 mt-6 sm:mt-8' : 'order-3 mt-6 sm:mt-8'}`}>
      ...
      <div className={`${photoSize} rounded-full overflow-hidden border-2 ${borderOpacity} bg-white/10`}>
        {player.photo_url ? (
          <img src={player.photo_url} alt={player.name} width="80" height="80" loading="lazy" className="w-full h-full object-cover" />
        ) : (
      ...
      {/* Club logo */}
      {clubLogo && (
        <img src={clubLogo} alt={player.club} width="20" height="20" loading="lazy" className="w-4 h-4 sm:w-5 sm:h-5 mt-1 object-contain" />
      )}
